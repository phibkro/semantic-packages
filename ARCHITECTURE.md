...
## System planes and delivery map

The [tracer system map](docs/design/system-map.md) connects the canonical product data
plane, product decision plane, and project/agent control plane layer by layer. It also
marks which tracer increments are executable, designed, or still planned and maps the
nested artifact, Evidence, and project-change lifecycles.

That map is a revisable orientation projection. The entity and compatibility rules in
this document and the linked concern documents remain normative; live milestone status
and retained failures remain in the active ExecPlan.
