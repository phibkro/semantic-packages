...
| PR-M1 checkout runtime maintenance | lead CI maintainer; depends on merged JG and the hosted Node 20 deprecation observation; exclusive `.github/workflows/quality-gates.yml` plus this plan | local acceptance checkpoint: replace both v4.2.2 pins with official latest v7.0.0 commit `9c091bb21b7c1c1d1991bb908d89e4e9dddfe3e0`; upstream `action.yml` declares `node24`, both existing fetch-depth and no-credential boundaries remain unchanged, and diff hygiene, actionlint from locked nixpkgs, flake check, and the full repository gate pass; the follow-up PR owns hosted warning removal and gate reproduction |
...
- [ ] PR-M1 checkout Node 24 maintenance convergence
...
- That final hosted run warned that checkout v4.2.2's Node 20 runtime is deprecated.
  ...
  check, and the full repository gate pass; hosted warning removal remains the
  follow-up PR's decisive observation.
...
therefore accepted. PR-M1 is the bounded post-release maintenance successor for the
observed checkout Node 20 deprecation; it changes no semantic-package behavior.
