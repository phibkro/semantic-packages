...
| W4-R1 Rust Realization/adapter | depends on W4-CF1; independent implementation owner plus lead protocol successor | exclusive `implementations/rust/**` | implementation checkpoint green: independent immutable bottom-first boxed-slice Stack, std-only adapter, direct reproducible rustc build, exact campaign support, and arbitrary canonical-decimal sequence echo; retained codec dissent limits this to bounded valid v1 requests |
| W4-T1 TypeScript Realization/adapter | depends on W4-CF1; independent implementation owner plus lead protocol successor | exclusive `implementations/typescript/**` | accepted successor checkpoint: independent frozen linked-node Stack and offline Deno adapter pass exact campaign and arbitrary canonical-decimal sequence echo after TR1 rejected the initial safe-integer restriction |
| W4-RR1 Rust review | challenges initial W4-R1 | read-only independence, codec, semantics, lifecycle, and provenance audit | PASS on the exact campaign, novel persistence/framing/member-order probes, byte-reproducible build, and representation independence; its bounded acceptance of i64 sequence numbers was later corrected consistently with TR1 and therefore requires RR2 |
| W4-RR2 Rust/record successor review | depends on the sequence successor and W4-EG1 packet | read-only revised Rust protocol plus canonical record/Evidence audit | PASS: reproducible direct-rustc build, arbitrary canonical-decimal sequences, exact campaign, and current source/report bindings; std-only codec and lexical sequence boundary remain explicit limits |
| W4-TR1 TypeScript review | challenges initial W4-T1 | read-only independence, integer/runtime, semantics, lifecycle, and provenance audit | BLOCK: valid sequence `9007199254740993` was rejected by an implementation-only JavaScript safe-integer limit; all bounded campaign semantics otherwise passed |
| W4-TR2 TypeScript successor review | challenges/supports corrected W4-T1 | read-only blocker replay, arbitrary integer, framing, independence, and campaign audit | PASS: 14-test matrix, exact campaign, escaped/reordered keys, split/combined input, 16/81/401-digit monotonic sequences, byte-exact echo, nested-key rejection, and clean EOF pass; canonical decimal spelling remains an explicit protocol assumption |
| W4-EG1 cross-language record/Evidence gate | depends on accepted W4-R1/T1 reviews and G1 | lead canonical graph/provenance integration owner | rejected checkpoint: schemas, links, digests, report contents, and declaration scope pass, but RR2 blocks on the obsolete flat fixture-only triple, incomplete executable argv/input provenance, no fresh report/Evidence checker in G0, and pre-successor Rust review attribution |
| W4-EG2 record/Evidence successor | depends on W4-EG1/RR2 | same lead canonical graph/provenance integration owner | rejected checkpoint: obsolete triple relocation and executable provenance close, but successor review proves accepted Evidence can still be rebound to another valid Claim/declaration or altered bounds/environment/assumptions/exclusions while the checker reports eight records bound |
| W4-EG3 declaration-binding successor | depends on rejected W4-EG2 review | same lead canonical graph/provenance integration owner | rejected checkpoint: Claim/Evidence/declaration/scope/environment mutations now fail, but successor review changes the canonical Rust Realization to browser TypeScript plus a direct-interoperation claim without any schema, graph, campaign, or Evidence-check failure |
| W4-EG4 Realization-binding successor | depends on rejected W4-EG3 review | same lead canonical graph/provenance integration owner | rejected checkpoint: Realization metadata mutations now fail, but `W4-RR2 BLOCK: ...` still satisfies the substring review check and unexpected provenance fields remain accepted |
| W4-EG5 exact-review/provenance successor | depends on rejected W4-EG4 review | same lead canonical graph/provenance integration owner | PASS: exact review dispositions and provenance member sets are bound; a durable mutation rejects contradictory `BLOCK` text and an invented interoperability field; reviewer probes reject Claim, Evidence, Realization, source, matrix, report, tool, binary, and disposition rebinding |
| W4-G1 independent-realization convergence | depends on W4-HG1, W4-RR2, W4-TR2, W4-EG5, and G0 | lead acceptance owner | accepted: 18 candidate controls, two freshly reproduced reports, eight exact-bound Evidence records, and the full repository gate pass; retained limits remain visible |
...
enables format assertion explicitly. The current Wave 4 gate also runs:
...
python3 -m unittest tests.candidates.test_cross_language_candidates
python3 scripts/wave4_evidence_check.py
...
Expected repository observation: `Record fixture checks passed: 24 valid, 20
schema-invalid, 38 link-invalid, 3 link-valid.`, `Loader fixture checks passed: 18
contract groups.`, `Adapter fixture checks passed: 36 tests.`, `Cross-language
candidate checks passed: 18 tests.`, `Wave 4 Evidence checks passed: 2 fresh reports
and 8 records.`, `Proof fixture checks passed: 49 contract groups.`, and `Repository
checks passed.` with exit status 0. The
...
- W4-CF1R/T supply independent external negative packages before correct candidates.
  ...
  at the red checkpoint six fixture/toolchain tests passed, while six correct-candidate
  tests failed only because `implementations/{rust,typescript}` source files did not yet
  exist. Raw host store
  paths remain run provenance, not repository build instructions.
- W4-R1/RR1 produced an independent boxed-slice Rust realization and reproducible
  direct-rustc adapter. W4-T1 initially passed the bounded campaign, but TR1 correctly
  rejected its silent JavaScript safe-integer sequence limit against the unbounded
  protocol wording. The successor uses canonical decimal sequence representations in
  both languages, adds durable greater-than-`2^53` controls, and TR2 passes values up
  to 401 digits. Alternate mathematically integral JSON spellings such as `1.0`, `1e3`,
  and `-0` remain a named lexical assumption and reopen trigger rather than an inferred
  extension.
- W4-EG1 retains compact Rust and TypeScript campaign reports as provenance artifacts,
  not Evidence by themselves. Lead integration authored two canonical Realizations,
  eight declaration-scoped Claims, and eight exact-plan Evidence records whose accepted
  review state, assumptions, exclusions, source/runner/tool digests, and profile bounds
  remain explicit. RR2 rejected the first packet because schema validity did not make
  its reports reproducible or remove the obsolete fixture-only support from the flat
  set. EG2 retains that unchanged triple in an isolated link-valid historical fixture,
  adds exact executable/input provenance, and makes G0 freshly reproduce the candidate
  controls, two reports, and eight records. EG2 was still rejected after a Claim swap
  and scope/bounds mutation remained falsely accepted; EG3 binds every Claim/Evidence
  field to its named declaration, but review then exposes unbound Realization metadata.
  EG4 binds each Realization to its actual adapter/runtime and extends the permanent
  mutation control to reject false language/runtime/interoperation claims, but its
  substring review check still accepts contradictory `BLOCK` text. EG5 exact-binds
  review dispositions and provenance members, retains that bypass as a durable
  mutation control, and passes independent rebinding probes. The flat gate now counts
  24 product-capable records; historical fixture data remains outside that set.
- A read-only four-actor journey audit finds the executable authoring/loading/proof and
  package-author conformance substrate strong but the product slice incomplete: no
  curated registry source set, resolver, browser/projection, or executable maintenance
  recovery journey exists. The next plan should make actor journey IDs the primary
  acceptance surface: theory-author local publication, independent package registration,
  one honest graph, parallel package-consumer resolution and theory-consumer browse,
  then exact-version successor/staleness recovery. For this tracer, registry means the
  finite local exact-version source set; hosted acquisition remains excluded.
- A diverse read-only Claude Fable 5/high lifecycle skepticism node passes the scalable
  DAG-node model with concerns. It distinguishes phase clusters from the invariant node
  spine (intent, falsifier, ownership boundary, observation, retained disposition),
  recommends containment-first security handling, successor-only gate changes,
  blocking independent review when an acceptance owner authored the consequential gate,
  and escalation for weakened acceptance. It also identifies unchecked append-only
  governance memory and systematic provider-fallback drift as reopen candidates. The
  dispatch completed through `agent-dispatch --read-only`, pagu-box strict, with public
  governing documents only; runtime reported `claude-fable-5` plus auxiliary
  `claude-haiku-4-5-20251001`, ten turns, no writes, network, or delegation.
- GitHub governance preflight finds only `.github/workflows/quality-gates.yml`; PR #3
  has one failing quality-gate run and no visible CodeRabbit or Claude review activity.
  Existing PR descriptions are comprehensive, but PR titles and published stacked
  commit histories predate Conventional Commits. Connector title updates were denied
  with HTTP 403 and local `gh` authentication is invalid, so repository merge settings,
  rulesets, title normalization, and any history rewrite require operator access. The
  next governance node should add education plus deterministic PR-title/commit checks,
  then make only squash merge available and require the checks; model reviews remain
  advisory and least-privilege rather than acceptance authority.
...
substrate. Wave 4 now has a hardened exact-plan campaign, two independently represented
Rust and TypeScript Realizations, protocol-correct negative controls, and
declaration-scoped reviewed Evidence. W4-G1 closes after four rejected Evidence-binding
checkpoints: the exact repository gate now passes 24 canonical records, 18 candidate
controls, two freshly reproduced reports, eight accepted Evidence records, 36 adapter
tests, and the retained proof/loader gates. No compatibility resolver, curated product
registry projection, browser, or maintenance-recovery journey has started.
