...
When `acceptedMechanisms` is present it is a closed exact set, including the empty set.
When absent, mechanism acceptance is unspecified and no Evidence can satisfy the entry;
absence is not a wildcard. The resolver retains that policy-incomplete explanation.
See [ADR 0013](../decisions/0013-bounded-policy-mechanism-and-scope.md).
...
The Stack product's provisional minimum-assurance token derives required declarations
from the exact Specification before selecting Claims, so a missing Claim cannot satisfy
an empty set vacuously. An omitted or unknown token is policy-incomplete and fail-closed.
Review state, result, applicability, mechanism, observation scope, and assurance remain
separately evaluated axes; see ADR 0013.
...
For the Stack tracer, prohibition observation-scope tokens are classified by the exact
governed Evidence mechanism and provenance boundary rather than prose matching. The
`adapter-invocation-trace` rule and its limitations are fixed by ADR 0013.
