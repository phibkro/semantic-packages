...
Policy sets are fail-closed. A present empty `acceptedMechanisms` accepts none; an
omitted field leaves mechanism acceptance unspecified rather than permitting every
mechanism. Observation-scope tokens match only a governed mechanism/provenance boundary,
not free-form prose. For the bounded Stack prohibition, `adapter-invocation-trace`
selects only exact-bound Wave 4 effect Evidence and retains the event-completeness
assumption and external-effects exclusion described in
[ADR 0013](../decisions/0013-bounded-policy-mechanism-and-scope.md).
Scope classification is independent of Evidence review state and result. For the Stack
campaign it is computed from exact Claim/Specification/profile/Realization/adapter,
mechanism, declaration, and plan-digest fields in the canonical graph. The independent
W4 reproduction supports release acceptance; it is not runtime resolver state.
