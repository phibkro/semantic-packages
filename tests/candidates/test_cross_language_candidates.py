...
from scripts import wave4_evidence_check
...
class CrossLanguageCandidateContractTests(unittest.TestCase):
    ...
    def _assert_arbitrary_sequence_integer(self, child_argv: Sequence[str]) -> None:
        process = subprocess.Popen(
            tuple(child_argv),
            cwd=ROOT,
            shell=False,
    ...
    def _binding_records(
        self, candidate: str, declaration: str
    ) -> tuple[dict[str, object], dict[str, object]]:
        prefix = "stack-rust" if candidate == "rust" else "stack-typescript"
        evidence = json.loads(
            (ROOT / "fixtures/records/valid" / f"{prefix}-{declaration}-evidence.json")
            .read_text(encoding="utf-8")
        )
        claim = json.loads(
            (ROOT / "fixtures/records/valid" / f"{prefix}-{declaration}-claim.json")
            .read_text(encoding="utf-8")
        )
        return evidence, claim

    def test_wave4_evidence_records_bind_every_declaration(self) -> None:
        for candidate in ("rust", "typescript"):
            for declaration in DECLARATIONS:
                with self.subTest(candidate=candidate, declaration=declaration):
                    evidence, claim = self._binding_records(candidate, declaration)
                    self.assertEqual(
                        [],
                        wave4_evidence_check._binding_errors(
                            candidate, declaration, evidence, claim
                        ),
                    )

    def test_wave4_evidence_rejects_claim_proposition_swap(self) -> None:
        evidence, claim = self._binding_records("rust", "pop-empty")
        mutated = copy.deepcopy(claim)
        mutated["proposition"]["declarationId"] = "pop-push"
        self.assertTrue(
            wave4_evidence_check._binding_errors(
                "rust", "pop-empty", evidence, mutated
            )
        )

    def test_wave4_evidence_rejects_claim_and_profile_rebinding(self) -> None:
        evidence, claim = self._binding_records("typescript", "persistence")
        mutated = copy.deepcopy(evidence)
        mutated["claim"]["id"] = "stack-typescript-pop-push"
        mutated["applicability"]["parameters"]["maximumDepth"] = 7
        errors = wave4_evidence_check._binding_errors(
            "typescript", "persistence", mutated, claim
        )
        self.assertTrue(any("claim" in error for error in errors))
        self.assertTrue(any("applicability" in error for error in errors))

    def test_wave4_evidence_rejects_scope_assumption_and_exclusion_drift(self) -> None:
        evidence, claim = self._binding_records("rust", "stack-effects")
        mutated = copy.deepcopy(evidence)
        mutated["scope"] = "specification"
        mutated["environment"]["rustc"] = "1.96.2"
        mutated["assumptions"] = ["report was green"]
        mutated["exclusions"] = []
        errors = wave4_evidence_check._binding_errors(
            "rust", "stack-effects", mutated, claim
        )
        ...
        self.assertTrue(any("exclusions" in error for error in errors))

        realization = json.loads(
            (ROOT / "fixtures/records/valid/stack-rust-realization.json").read_text(
                encoding="utf-8"
            )
        )
        realization["implementation"]["language"] = "TypeScript"
        realization["implementation"]["runtime"] = "browser JavaScript"
        realization["limitations"] = ["All languages interoperate directly."]
        realization_errors = wave4_evidence_check._realization_binding_errors(
            "rust", realization
        )
        self.assertTrue(any("implementation" in error for error in realization_errors))
        self.assertTrue(any("limitations" in error for error in realization_errors))

        contradictory = copy.deepcopy(evidence["provenance"])
        contradictory["review"] = "W4-RR2 BLOCK: successor rejected current source"
        contradictory["interoperability"] = "all languages compose directly"
        review_errors = wave4_evidence_check._review_binding_errors(
            "rust", "stack-effects", contradictory
        )
        self.assertIn("review provenance changed", review_errors)
        self.assertIn("provenance members changed", review_errors)
    ...
    def test_rust_correct_candidate_preserves_arbitrary_sequence_integer(self) -> None:
        with tempfile.TemporaryDirectory(prefix="wave4-rust-candidate-seq-") as directory:
            child_argv = self._build_rust(
                RUST_CANDIDATE, Path(directory) / "stack-rust-adapter"
            )
            self._assert_arbitrary_sequence_integer(child_argv)

    def test_typescript_correct_candidate_preserves_arbitrary_sequence_integer(self) -> None:
        self._assert_arbitrary_sequence_integer(
            self._deno_child_argv(TYPESCRIPT_CANDIDATE)
        )
